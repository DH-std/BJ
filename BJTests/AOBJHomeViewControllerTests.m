...
describe(@"APBJViewController", ^{
    ...
    it (@"buttonTapped sets the wasTapped variable", ^{
        [[subject shouldNot] beNil];
        
        [subject buttonTapped: nil];
        
        // Need to wrap wasTapped in theValue() because it is a BOOL, not an object
        [[theValue(subject.wasTapped) should] beTrue];
    });
});
